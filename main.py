...
start_date_prediction = st.date_input("Data for previous date for MACD", value=DEFAULT_START_DATE_PREDICTION)
end_date_prediction = st.date_input("Prediction Date Data", value=DEFAULT_END_DATE_PREDICTION)
factor = st.number_input("Days-Factor-MACD", value=DEFAULT_FACTOR)
...
history = model.fit(X_train, y_train, epochs=10, batch_size=25, validation_split=0.2, callbacks=[early_stopping])
...
st.write("Predicted Prices for the next 4 days: ", predicted_stock_price)
